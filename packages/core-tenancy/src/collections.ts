...
export async function getPlatformSettingsRepository(): Promise<PlatformSettingsRepository> {
  const db = await getDb();
  return new PlatformSettingsRepository(
    db.collection<PlatformSettingsDoc>("platform_settings"),
  );
}
