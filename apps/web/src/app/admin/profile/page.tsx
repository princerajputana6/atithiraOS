    ...
    <div>
      ...
      <Card>
        <div className="flex items-center gap-4">
          <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-fuchsia-400 to-indigo-400 text-xl font-bold text-slate-950">
            {(owner.email[0] ?? "A").toUpperCase()}
          </span>
          <div>
            <p className="text-lg font-semibold text-white">{owner.email}</p>
            <p className="text-sm text-slate-400">Platform Owner · Atithira staff</p>
          </div>
        </div>
        ...
        <dl className="mt-6 grid grid-cols-1 gap-4 border-t border-white/10 pt-6 sm:grid-cols-2">
          <div>
            ...
            <dd className="mt-1 text-sm text-slate-200">{owner.email}</dd>
          </div>
          <div>
            <dt className="text-xs font-medium uppercase tracking-wider text-slate-500">Scope</dt>
            <dd className="mt-1 text-sm text-slate-200">Global — all tenants</dd>
          </div>
        </dl>
